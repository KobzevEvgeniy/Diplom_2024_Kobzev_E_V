...
#     formula_item = Ingredient.objects
#     formula_count = formula_item.count()
#     if formula_count <= 0:
#         return redirect('new_formula')
#     if request.method == 'POST':
#         formula = Formula(request.POST)
#         formula_ingredient = ArrayField(formula_item(request.POST))
# 
#         if formula.is_valid():
#             data = Formula()
...
def new_formula(request, total_dry_matter_product=None, total_price_product=None, formula_count=0):
    current_user = request.user
    formula_item = Ingredient.objects
    formula_count = formula_item.count()
    if formula_count <= 0:
    ...
    if request.method == 'POST':
        ...
        if formula.is_valid():
            ...
            data.save()
            formula_ingredient.formula = data
            formula_ingredient.ingredient = formula_ingredient.choese(formula_item)
            formula_ingredient.brix = formula_ingredient.brix
            formula_ingredient.quantity_1 = formula_ingredient.cleaned_data['quantity_1']
            formula_ingredient.unit_of_measure = formula_ingredient.unit_of_measure
            formula_ingredient.dry_matter = formula_ingredient.brix * formula_ingredient.quantity_1
        ...
        context = {
                ...
                'name_product': formula.name_product,
                'status_success': formula.status_success,
                'stage': formula_item.stage,
                'ingredient': formula_item.name,
                'BRIX': formula_item.brix,
                'total_dry_matter_product': total_dry_matter_product,
                'quantity_1': formula_item.quantity_1,
                'unit_of_measure': formula_item.unit_of_measure,
                'price': formula_item.price,
                'level_in_total_volume': formula_item.level_in_total_volume,
                'total_price_product': total_price_product,
                'description_formula': formula.description_formula,
                'description_product': formula.description_product
