...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            profile.profile_picture = 'media/user_pictures/default-user.png'
            profile.save()
            #
            # # USER ACTIVATION
            # current_site = get_current_site(request)
            # mail_subject = 'Активируйте свой аккаунт в Technologist table'
        ...
        if user is not None:
            try:
                ...
                if is_cabinet_item_exists:
                    ...
                    for ingred_var in formula_variation:
                        if ingred_var in existing_variation_list:
                            item_index = existing_variation_list.index(ingred_var)
...
def my_formulas(request):
    try:
        ...
        return render(request, 'accounts/my_formulas.html', context)
    except(TypeError, ValueError, OverflowError, ObjectDoesNotExist):
        return render(request, 'accounts/my_formulas.html')
...
def formula_detail(request, formula_id):
    ...
    return render(request, 'accounts/formula_detail.html', context)
